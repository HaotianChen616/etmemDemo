...
如果当前机器没有适合 cslide 的 VM hugepage / 分级内存环境，再使用下面的 raw scan fallback。

## 3. 第二阶段：底层扫描 fallback

这个阶段只读 `/proc/<pid>/idle_pages`，用于确认 etmem_scan 内核接口可用。它不等价于完整 etmem 分析流程。
